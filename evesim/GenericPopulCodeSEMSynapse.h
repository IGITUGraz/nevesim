#ifndef GenericPopulCodeSEMSynapse_H_
#define GenericPopulCodeSEMSynapse_H_
...
template<class Base>
class GenericPopulCodeSEMSynapse : public Base {

public:
	GenericPopulCodeSEMSynapse(float Winit,
					float eta,
					bool withHessian,
	...
	virtual ~GenericPopulCodeSEMSynapse() {};
...
template<class Base>
void GenericPopulCodeSEMSynapse<Base>::eventHit(SimContext & sim_ctxt, Time time, port_id_t port) {
	...
	if (port == 1) {
		if (activePlasticity) {
			...
			double newW = Base::W + eta * m * (pre_trace - exp(Wpar));
			if (newW > maxW)
				Base::W = maxW;
			else
			if (newW < minW)
...
#endif /* GenericPopulCodeSEMSynapse_H_ */
